...
const createMockRouter = (): Router => {
  ...
  const routes: Partial<RouteRecordNormalized>[] = [
    ...
    { path: '/admin/risk-control', components: { default: mockImportFn } },
...
describe('useRoutePrefetch', () => {
  ...
  describe('_isAdminRoute', () => {
    ...
    it('应该正确识别非后台路由', () => {
      const { _isAdminRoute } = useRoutePrefetch(mockRouter)
      expect(_isAdminRoute('/login')).toBe(false)
      expect(_isAdminRoute('/setup')).toBe(false)
      expect(_isAdminRoute('/unknown-route')).toBe(false)
    })
  })
  ...
  describe('_getPrefetchConfig', () => {
    ...
    it('Settings 页面应该返回正确的预加载配置', () => {
      const { _getPrefetchConfig } = useRoutePrefetch(mockRouter)
      const route = createMockRoute('/admin/settings')
      const config = _getPrefetchConfig(route)
  ...
  describe('预加载映射表', () => {
    ...
    it('兼容映射表应该只包含轻量网关后台路由', () => {
      expect(_userPrefetchMap).toHaveProperty('/admin/settings')
      expect(_userPrefetchMap['/admin/settings']).toHaveLength(2)
    })
  })
