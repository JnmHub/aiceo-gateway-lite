...
This directory contains all Pinia stores for the JnmGatewayApi frontend application.

## Stores Overview

### 1. Auth Store (`auth.ts`)

Manages gateway-lite（轻量网关）administrator authentication state, login/logout, token refresh, and token persistence.
...
- `login(credentials)` - Authenticate administrator with email/password
- `login2FA(tempToken, totpCode)` - Complete TOTP（双因素认证）login
