...
const (
	...
	// RunModeGatewayLite runs the regional API gateway plus the minimal
	// operations admin surface. Customer self-service, payment, and signup
	// concerns live in a separate control plane.
	RunModeGatewayLite = "gateway-lite"
)
...
type Config struct {
	...
	GatewayLite             GatewayLiteConfig             `mapstructure:"gateway_lite"`
...
type GatewaySchedulingConfig struct {
	...
	FallbackQueueEnabled bool          `mapstructure:"fallback_queue_enabled"`
...
// GatewayLiteConfig 是区域轻量网关连接主站控制面的启动配置。
type GatewayLiteConfig struct {
	Region                           string   `mapstructure:"region" yaml:"region"`
	GatewayCode                      string   `mapstructure:"gateway_code" yaml:"gateway_code"`
	AvailableModels                  []string `mapstructure:"available_models" yaml:"available_models"`
	RedisPrefix                      string   `mapstructure:"redis_prefix" yaml:"redis_prefix"`
	AdminSyncKey                     string   `mapstructure:"admin_sync_key" yaml:"admin_sync_key"`
	ControlPlaneURL                  string   `mapstructure:"control_plane_url" yaml:"control_plane_url"`
	ControlPlaneToken                string   `mapstructure:"control_plane_token" yaml:"control_plane_token"`
	ControlPlaneTimeoutMS            int      `mapstructure:"control_plane_timeout_ms" yaml:"control_plane_timeout_ms"`
	RuntimeHealthIntervalSeconds     int      `mapstructure:"runtime_health_interval_seconds" yaml:"runtime_health_interval_seconds"`
	RuntimeActiveWindowSeconds       int      `mapstructure:"runtime_active_window_seconds" yaml:"runtime_active_window_seconds"`
	ConfigSyncIntervalSeconds        int      `mapstructure:"config_sync_interval_seconds" yaml:"config_sync_interval_seconds"`
	CacheInvalidationIntervalSeconds int      `mapstructure:"cache_invalidation_interval_seconds" yaml:"cache_invalidation_interval_seconds"`
	UsageQueuePendingAlertThreshold  int64    `mapstructure:"usage_queue_pending_alert_threshold" yaml:"usage_queue_pending_alert_threshold"`
	UsageQueueDeadAlertThreshold     int64    `mapstructure:"usage_queue_dead_alert_threshold" yaml:"usage_queue_dead_alert_threshold"`
}
...
func NormalizeRunMode(value string) string {
	...
	switch normalized {
	case RunModeStandard, RunModeSimple, RunModeGatewayLite:
		return normalized
	default:
...
func load(allowMissingJWTSecret bool) (*Config, error) {
	...
	cfg.GatewayLite.Region = strings.TrimSpace(cfg.GatewayLite.Region)
	cfg.GatewayLite.GatewayCode = strings.TrimSpace(cfg.GatewayLite.GatewayCode)
	cfg.GatewayLite.RedisPrefix = strings.TrimSpace(cfg.GatewayLite.RedisPrefix)
	cfg.GatewayLite.ControlPlaneURL = strings.TrimSpace(cfg.GatewayLite.ControlPlaneURL)
	cfg.GatewayLite.ControlPlaneToken = strings.TrimSpace(cfg.GatewayLite.ControlPlaneToken)
	if cfg.GatewayLite.ControlPlaneTimeoutMS <= 0 {
		cfg.GatewayLite.ControlPlaneTimeoutMS = 300
	}
	if cfg.GatewayLite.RuntimeHealthIntervalSeconds <= 0 {
		cfg.GatewayLite.RuntimeHealthIntervalSeconds = 15
	}
	if cfg.GatewayLite.RuntimeActiveWindowSeconds <= 0 {
		cfg.GatewayLite.RuntimeActiveWindowSeconds = 300
	}
	if cfg.GatewayLite.ConfigSyncIntervalSeconds <= 0 {
		cfg.GatewayLite.ConfigSyncIntervalSeconds = 30
	}
	if cfg.GatewayLite.CacheInvalidationIntervalSeconds <= 0 {
		cfg.GatewayLite.CacheInvalidationIntervalSeconds = 5
	}
	if cfg.GatewayLite.UsageQueuePendingAlertThreshold <= 0 {
		cfg.GatewayLite.UsageQueuePendingAlertThreshold = 1000
	}
	if cfg.GatewayLite.UsageQueueDeadAlertThreshold <= 0 {
		cfg.GatewayLite.UsageQueueDeadAlertThreshold = 1
	}
...
func setDefaults() {
	viper.SetDefault("run_mode", RunModeGatewayLite)
	...
	// Admin bootstrap credentials. They are only used when the user table is empty.
	...
	// Gateway-lite control plane connection.
	viper.SetDefault("gateway_lite.region", "default")
	viper.SetDefault("gateway_lite.gateway_code", "")
	viper.SetDefault("gateway_lite.available_models", []string{})
	viper.SetDefault("gateway_lite.redis_prefix", "")
	viper.SetDefault("gateway_lite.admin_sync_key", "")
	viper.SetDefault("gateway_lite.control_plane_url", "")
	viper.SetDefault("gateway_lite.control_plane_token", "")
	viper.SetDefault("gateway_lite.control_plane_timeout_ms", 300)
	viper.SetDefault("gateway_lite.runtime_health_interval_seconds", 15)
	viper.SetDefault("gateway_lite.runtime_active_window_seconds", 300)
	viper.SetDefault("gateway_lite.config_sync_interval_seconds", 30)
	viper.SetDefault("gateway_lite.cache_invalidation_interval_seconds", 5)
	viper.SetDefault("gateway_lite.usage_queue_pending_alert_threshold", 1000)
	viper.SetDefault("gateway_lite.usage_queue_dead_alert_threshold", 1)
	...
	viper.SetDefault("gateway.scheduling.fallback_wait_timeout", 120*time.Second)
	viper.SetDefault("gateway.scheduling.fallback_queue_enabled", true)
...
func (c *Config) Validate() error {
	...
	if c.Gateway.Scheduling.FallbackQueueEnabled && c.Gateway.Scheduling.FallbackMaxWaiting <= 0 {
		return fmt.Errorf("gateway.scheduling.fallback_max_waiting must be positive when fallback queue is enabled")
	}
