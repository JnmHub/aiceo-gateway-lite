...
func TestLoadDefaultSchedulingConfig(t *testing.T) {
	...
	if cfg.Gateway.Scheduling.FallbackWaitTimeout != 120*time.Second {
		t.Fatalf("FallbackWaitTimeout = %v, want 120s", cfg.Gateway.Scheduling.FallbackWaitTimeout)
	}
	if !cfg.Gateway.Scheduling.FallbackQueueEnabled {
		t.Fatal("FallbackQueueEnabled = false, want true")
	}
