...
// RegisterGatewayLiteAuthRoutes 注册 gateway-lite（轻量网关）后台需要的最小登录认证路由。
// 这里故意不注册注册、找回密码、OAuth 登录、邀请码和优惠码等客户侧入口，
// 避免区域网关暴露主站自助业务面。
func RegisterGatewayLiteAuthRoutes(
	v1 *gin.RouterGroup,
	h *handler.Handlers,
	jwtAuth servermiddleware.JWTAuthMiddleware,
	redisClient *redis.Client,
	...
	auth.Use(servermiddleware.BackendModeAuthGuard(settingService))
	{
		auth.POST("/login", rateLimiter.LimitWithOptions("gateway-lite-auth-login", 20, time.Minute, middleware.RateLimitOptions{
			FailureMode: middleware.RateLimitFailClose,
		}), h.Auth.Login)
		auth.POST("/login/2fa", rateLimiter.LimitWithOptions("gateway-lite-auth-login-2fa", 20, time.Minute, middleware.RateLimitOptions{
			FailureMode: middleware.RateLimitFailClose,
		}), h.Auth.Login2FA)
		auth.POST("/refresh", rateLimiter.LimitWithOptions("gateway-lite-refresh-token", 30, time.Minute, middleware.RateLimitOptions{
			FailureMode: middleware.RateLimitFailClose,
		}), h.Auth.RefreshToken)
		auth.POST("/logout", h.Auth.Logout)
	}
