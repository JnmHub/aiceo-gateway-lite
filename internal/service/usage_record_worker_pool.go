...
func (p *UsageRecordWorkerPool) Stats() UsageRecordWorkerPoolStats {
	...
	if p.queueSize > 0 {
		queueUtilizationPct = int((waitingTasks * 100) / uint64(p.queueSize))
		if queueUtilizationPct > 100 {
	...
	return UsageRecordWorkerPoolStats{
		...
		QueueUtilizationPct:   queueUtilizationPct,
		OverflowPolicy:        p.overflowPolicy,
		OverflowSamplePercent: p.overflowSamplePercent,
		TaskTimeoutSeconds:    int64(p.taskTimeout / time.Second),
		AutoScaleEnabled:      p.autoScaleEnabled,
		AutoScaleMinWorkers:   p.autoScaleMinWorkers,
		AutoScaleMaxWorkers:   p.autoScaleMaxWorkers,
		AutoScaleUpPercent:    p.autoScaleUpPercent,
		AutoScaleDownPercent:  p.autoScaleDownPercent,
		AutoScaleUpStep:       p.autoScaleUpStep,
		AutoScaleDownStep:     p.autoScaleDownStep,
		AutoScaleIntervalMS:   p.autoScaleInterval.Milliseconds(),
		AutoScaleCooldownMS:   p.autoScaleCooldown.Milliseconds(),
...
func (s UsageRecordWorkerPoolStats) String() string {
	return fmt.Sprintf("running=%d waiting=%d queue=%d%% submitted=%d dropped=%d", s.RunningWorkers, s.WaitingTasks, s.QueueUtilizationPct, s.SubmittedTasks, s.DroppedTasks)
}
