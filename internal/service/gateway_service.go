...
// ErrFallbackQueueFull 表示账号并发已满且兜底等待队列已满。
var ErrFallbackQueueFull = errors.New("fallback queue full")
...
func (s *GatewayService) SelectAccountWithLoadAwareness(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, metadataUserID string, sub2apiUserID int64) (*AccountSelectionResult, error) {
	...
	for _, acc := range candidates {
		if !s.canUseFallbackQueue(ctx, cfg, acc.ID) {
			continue
		}
		// 会话数量限制检查（等待计划也需要占用会话配额）
...
func (s *GatewayService) applyFallbackQueueSettings(ctx context.Context, cfg config.GatewaySchedulingConfig) config.GatewaySchedulingConfig {
...
func (s *GatewayService) canUseFallbackQueue(ctx context.Context, cfg config.GatewaySchedulingConfig, accountID int64) bool {
...
func (s *GatewayService) selectAccountForModelWithPlatform(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, platform string) (*Account, error) {
	...
	if groupID != nil && s.groupRepo != nil && (s.cfg == nil || s.cfg.RunMode != config.RunModeGatewayLite) {
...
type gatewayLiteUsageIdentityResolver interface {
	ResolveGatewayLiteUsageIdentity(ctx context.Context, remoteUserID int64, remoteKeyID string) (userID int64, apiKeyID int64, err error)
}
...
func writeUsageLogBestEffort(ctx context.Context, repo UsageLogRepository, usageLog *UsageLog, logKey string, cfgs ...*config.Config) {
	...
	defer cancel()
	if !resolveGatewayLiteUsageLogIdentity(usageCtx, repo, usageLog, logKey, cfgs...) {
		return
	}
...
func resolveGatewayLiteUsageLogIdentity(ctx context.Context, repo UsageLogRepository, usageLog *UsageLog, logKey string, cfgs ...*config.Config) bool {
	if usageLog == nil || usageLog.APIKeyID > 0 || !usageLogNeedsGatewayLiteIdentity(cfgs...) {
		return true
	}

	resolver, ok := repo.(gatewayLiteUsageIdentityResolver)
	if !ok {
		logger.LegacyPrintf(logKey, "Create usage log skipped: gateway-lite remote key has no local identity resolver")
		return false
	}

	remoteUserID := usageLog.UserID
	remoteKeyID := ""
	if meta, ok := gatewaylite.RequestMetaFromContext(ctx); ok {
		remoteUserID = meta.UserID
		remoteKeyID = meta.KeyID
	}
	if strings.TrimSpace(remoteKeyID) == "" {
		remoteKeyID = strconv.FormatInt(remoteUserID, 10)
	}

	userID, apiKeyID, err := resolver.ResolveGatewayLiteUsageIdentity(ctx, remoteUserID, remoteKeyID)
	if err != nil {
		logger.LegacyPrintf(logKey, "Create usage log skipped: resolve gateway-lite local usage identity failed: %v", err)
		return false
	}
	if userID <= 0 || apiKeyID <= 0 {
		logger.LegacyPrintf(logKey, "Create usage log skipped: invalid gateway-lite local usage identity user=%d api_key=%d", userID, apiKeyID)
		return false
	}

	usageLog.UserID = userID
	usageLog.APIKeyID = apiKeyID
	usageLog.GroupID = nil
	usageLog.SubscriptionID = nil
	return true
}

func usageLogNeedsGatewayLiteIdentity(cfgs ...*config.Config) bool {
	for _, cfg := range cfgs {
		if cfg != nil && cfg.RunMode == config.RunModeGatewayLite {
			return true
		}
	}
	return false
...
func (s *GatewayService) GetAvailableModels(ctx context.Context, groupID *int64, platform string) []string {
	if s != nil && s.cfg != nil && s.cfg.RunMode == config.RunModeGatewayLite {
		groupID = nil
	}
