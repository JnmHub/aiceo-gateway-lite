...
func TestUsageRecordWorkerPool_StringHelpers(t *testing.T) {
	...
	stats := UsageRecordWorkerPoolStats{RunningWorkers: 2, WaitingTasks: 3, QueueUtilizationPct: 12, SubmittedTasks: 5, DroppedTasks: 1}
	require.Contains(t, stats.String(), "running=2")
	require.Contains(t, stats.String(), "waiting=3")
	require.Contains(t, stats.String(), "queue=12%")
}
...
func TestNewUsageRecordWorkerPool_FromConfig(t *testing.T) {
	...
	require.Equal(t, 16, stats.QueueCapacity)
	require.Equal(t, config.UsageRecordOverflowPolicyDrop, stats.OverflowPolicy)
	require.EqualValues(t, 2, stats.TaskTimeoutSeconds)
	require.False(t, stats.AutoScaleEnabled)
	require.Equal(t, 3, stats.AutoScaleMinWorkers)
	require.Equal(t, 3, stats.AutoScaleMaxWorkers)
}
