...
func TestOpenAIAccountEligibility_OAuthRejectsUnmappedThirdPartyModels(t *testing.T) {
	...
	require.False(t, isOpenAIAccountEligibleForRequest(ctx, &base, "mimo-v2.5-pro", false, ""))
	require.True(t, isOpenAIAccountEligibleForRequest(ctx, &base, "gpt-5.5", false, ""))
	require.True(t, isOpenAIAccountEligibleForRequest(ctx, &base, "codex-auto-review", false, ""))
}

func TestOpenAIAccountEligibility_APIKeyKeepsUnmappedCompatiblePassthrough(t *testing.T) {
	account := &Account{
		Platform:    PlatformOpenAI,
	...
	require.True(t, isOpenAIAccountEligibleForRequest(context.Background(), account, "mimo-v2.5-pro", false, ""))
}

func TestOpenAIAccountEligibility_OAuthAllowsExplicitAliasToOpenAIModelOnly(t *testing.T) {
	ctx := context.Background()
	account := &Account{
		Platform:    PlatformOpenAI,
		Type:        AccountTypeOAuth,
		...
		Credentials: map[string]any{
			"model_mapping": map[string]any{
				...
				"mimo-raw":      "mimo-v2.5-pro",
			},
		},
	}

	require.True(t, isOpenAIAccountEligibleForRequest(ctx, account, "mimo-v2.5-pro", false, ""))
	require.False(t, isOpenAIAccountEligibleForRequest(ctx, account, "mimo-raw", false, ""))
	require.False(t, isOpenAIAccountEligibleForRequest(ctx, account, "unknown-model", false, ""))
}

func TestSanitizeOpenAIUndefinedSentinelRequestBody(t *testing.T) {
	body := []byte(`{
		"model":"mimo-v2.5-pro",
		...
		"store":false,
		"input":[
			{"role":"user","content":[{"type":"input_text","text":"[undefined]"}]},
			{"role":"assistant","id":"[undefined]","content":[{"type":"output_text","text":"OK"}]}
		]
	}`)

	sanitized, modified, err := SanitizeOpenAIUndefinedSentinelRequestBody(body)
	require.NoError(t, err)
	require.True(t, modified)
	require.False(t, gjson.GetBytes(sanitized, "temperature").Exists())
	require.False(t, gjson.GetBytes(sanitized, "top_p").Exists())
	require.False(t, gjson.GetBytes(sanitized, "input.1.id").Exists())
	require.Equal(t, "[undefined]", gjson.GetBytes(sanitized, "input.0.content.0.text").String())
	require.Equal(t, "mimo-v2.5-pro", gjson.GetBytes(sanitized, "model").String())
}
