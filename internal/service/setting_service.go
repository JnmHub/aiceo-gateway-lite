...
type cachedFallbackQueueSettings struct {
...
const fallbackQueueSettingsCacheTTL = 5 * time.Second
...
type SettingService struct {
	...
	fallbackQueueSettingsCache        atomic.Value // *cachedFallbackQueueSettings
	fallbackQueueSettingsSF           singleflight.Group
}
...
func (s *SettingService) GetPublicSettings(ctx context.Context) (*PublicSettings, error) {
	...
	gatewayLite := runMode == config.RunModeGatewayLite
	...
	return &PublicSettings{
		...
		RunMode:                          runMode,
		UISurface:                        publicUISurface(runMode),
		GatewayLiteAdminEnabled:          gatewayLite,
		CustomerSelfServiceEnabled:       !gatewayLite,
		...
		SiteName:                         s.getStringOrDefault(settings, SettingKeySiteName, "JnmGatewayApi"),
		SiteLogo:                         settings[SettingKeySiteLogo],
		SiteSubtitle:                     s.getStringOrDefault(settings, SettingKeySiteSubtitle, "AI Gateway API Management"),
...
func publicUISurface(runMode string) string {
	if runMode == config.RunModeGatewayLite {
		return "gateway-lite-admin"
	}
	return "standard"
}
...
type PublicSettingsInjectionPayload struct {
	...
	GatewayLiteAdminEnabled          bool                     `json:"gateway_lite_admin_enabled"`
	CustomerSelfServiceEnabled       bool                     `json:"customer_self_service_enabled"`
...
func (s *SettingService) GetPublicSettingsForInjection(ctx context.Context) (any, error) {
	...
	return &PublicSettingsInjectionPayload{
		...
		UISurface:                        settings.UISurface,
		GatewayLiteAdminEnabled:          settings.GatewayLiteAdminEnabled,
		CustomerSelfServiceEnabled:       settings.CustomerSelfServiceEnabled,
...
func (s *SettingService) InitializeDefaultSettings(ctx context.Context) error {
	...
	defaults := map[string]string{
		...
		SettingKeySiteName:                                  "JnmGatewayApi",
		SettingKeySiteLogo:                                  "",
...
// GetFallbackQueueSettings 获取账号兜底排队配置
func (s *SettingService) GetFallbackQueueSettings(ctx context.Context) (*FallbackQueueSettings, error) {
	if s == nil {
		return DefaultFallbackQueueSettings(), nil
	}
	now := time.Now().UnixNano()
	if cached, ok := s.fallbackQueueSettingsCache.Load().(*cachedFallbackQueueSettings); ok && cached != nil {
	...
	result, err, _ := s.fallbackQueueSettingsSF.Do(SettingKeyFallbackQueueSettings, func() (any, error) {
		if cached, ok := s.fallbackQueueSettingsCache.Load().(*cachedFallbackQueueSettings); ok && cached != nil {
			if time.Now().UnixNano() < cached.expiresAt {
				settings := cached.settings
				return &settings, nil
		...
		settings, err := s.loadFallbackQueueSettingsFromRepo(ctx)
		if err != nil {
			return nil, err
		}
		s.fallbackQueueSettingsCache.Store(&cachedFallbackQueueSettings{
			settings:  *settings,
			expiresAt: time.Now().Add(fallbackQueueSettingsCacheTTL).UnixNano(),
		})
		return settings, nil
	})
	if err != nil {
	...
	if settings, ok := result.(*FallbackQueueSettings); ok && settings != nil {
		settingsCopy := *settings
		return &settingsCopy, nil
	}
	return s.defaultFallbackQueueSettingsFromConfig(), nil
}

func (s *SettingService) loadFallbackQueueSettingsFromRepo(ctx context.Context) (*FallbackQueueSettings, error) {
	value, err := s.settingRepo.GetValue(ctx, SettingKeyFallbackQueueSettings)
	if err != nil {
		if errors.Is(err, ErrSettingNotFound) {
			return s.defaultFallbackQueueSettingsFromConfig(), nil
		}
		return nil, fmt.Errorf("get fallback queue settings: %w", err)
	}
	if value == "" {
		return s.defaultFallbackQueueSettingsFromConfig(), nil
	}

	var settings FallbackQueueSettings
	if err := json.Unmarshal([]byte(value), &settings); err != nil {
		return s.defaultFallbackQueueSettingsFromConfig(), nil
	}

	s.normalizeFallbackQueueSettings(&settings)
	return &settings, nil
}

func (s *SettingService) normalizeFallbackQueueSettings(settings *FallbackQueueSettings) {
	if settings == nil {
		return
	}
	if settings.MaxWaiting < 1 {
		settings.MaxWaiting = 1
	}
	if settings.MaxWaiting > 100000 {
		settings.MaxWaiting = 100000
	}
	if settings.WaitTimeoutSeconds < 1 {
		settings.WaitTimeoutSeconds = s.defaultFallbackQueueSettingsFromConfig().WaitTimeoutSeconds
	}
	if settings.WaitTimeoutSeconds > 3600 {
		settings.WaitTimeoutSeconds = 3600
	}
}

func (s *SettingService) defaultFallbackQueueSettingsFromConfig() *FallbackQueueSettings {
	defaults := DefaultFallbackQueueSettings()
	if s != nil && s.cfg != nil {
		defaults.Enabled = s.cfg.Gateway.Scheduling.FallbackQueueEnabled
		if s.cfg.Gateway.Scheduling.FallbackMaxWaiting > 0 {
			defaults.MaxWaiting = s.cfg.Gateway.Scheduling.FallbackMaxWaiting
		}
		if s.cfg.Gateway.Scheduling.FallbackWaitTimeout > 0 {
			defaults.WaitTimeoutSeconds = int(s.cfg.Gateway.Scheduling.FallbackWaitTimeout.Seconds())
		}
	}
	return defaults
}

// SetFallbackQueueSettings 设置账号兜底排队配置
func (s *SettingService) SetFallbackQueueSettings(ctx context.Context, settings *FallbackQueueSettings) error {
	if settings == nil {
		return fmt.Errorf("settings cannot be nil")
	}

	if settings.MaxWaiting < 1 || settings.MaxWaiting > 100000 {
		if settings.Enabled {
			return fmt.Errorf("max_waiting must be between 1-100000")
		}
		settings.MaxWaiting = DefaultFallbackQueueSettings().MaxWaiting
	}
	if settings.WaitTimeoutSeconds < 1 || settings.WaitTimeoutSeconds > 3600 {
		if settings.Enabled {
			return fmt.Errorf("wait_timeout_seconds must be between 1-3600")
		}
		settings.WaitTimeoutSeconds = DefaultFallbackQueueSettings().WaitTimeoutSeconds
	}
	s.normalizeFallbackQueueSettings(settings)

	data, err := json.Marshal(settings)
	if err != nil {
		return fmt.Errorf("marshal fallback queue settings: %w", err)
	}

	if err := s.settingRepo.Set(ctx, SettingKeyFallbackQueueSettings, string(data)); err != nil {
		return err
	}
	s.fallbackQueueSettingsCache.Store(&cachedFallbackQueueSettings{
		settings:  *settings,
