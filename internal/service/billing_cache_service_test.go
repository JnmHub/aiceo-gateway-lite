...
func TestBillingCacheServiceRuntimeStats(t *testing.T) {
	svc := &BillingCacheService{cacheWriteChan: make(chan cacheWriteTask, 2)}
	svc.cacheWriteChan <- cacheWriteTask{kind: cacheWriteDeductBalance, userID: 1, amount: 1}
	svc.logCacheWriteDrop(cacheWriteTask{kind: cacheWriteDeductBalance, userID: 1}, "full")
	svc.logCacheWriteDrop(cacheWriteTask{kind: cacheWriteSetBalance, userID: 2}, "closed")
	svc.stopped.Store(true)

	stats := svc.RuntimeStats()
	require.Equal(t, 1, stats.QueueLength)
	require.Equal(t, 2, stats.QueueCapacity)
	require.Equal(t, cacheWriteWorkerCount, stats.WorkerCount)
	require.EqualValues(t, 1, stats.DroppedFullTotal)
	require.EqualValues(t, 1, stats.DroppedClosedTotal)
