			...
			if err != nil {
				...
				if errors.As(err, &promptTooLongErr) {
					...
					if !fallbackUsed && fallbackGroupID != nil && *fallbackGroupID > 0 {
						...
						if err := h.billingCacheService.CheckBillingEligibilityForRequest(c.Request.Context(), fallbackAPIKey.User, fallbackAPIKey, fallbackGroup, nil, service.PlatformFromAPIKey(fallbackAPIKey), service.EstimateRequestTokensForLimit(body)); err != nil {
							status, code, message, retryAfter := billingErrorDetails(err)
							if retryAfter > 0 {
								c.Header("Retry-After", strconv.Itoa(retryAfter))
							}
...
func (h *GatewayHandler) GlobalAvailableModels(ctx context.Context) []string {
	if h == nil || h.gatewayService == nil {
		return nil
	}
	return h.gatewayService.GetAvailableModels(ctx, nil, "")
}
