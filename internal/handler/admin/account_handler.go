...
func (h *AccountHandler) List(c *gin.Context) {
	...
	includeCredentials := lite && parseBoolQueryWithDefault(c.Query("include_credentials"), false)
	...
	for i := range accounts {
		...
		if includeCredentials && item.Account != nil {
			item.Account.Credentials = acc.Credentials
			item.Account.CredentialsStatus = nil
		}
	...
	etag := buildAccountsListETag(result, total, page, pageSize, platform, accountType, status, search, lite, includeCredentials)
	if etag != "" {
		c.Header("ETag", etag)
