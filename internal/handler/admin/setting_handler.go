...
// GET /api/v1/admin/settings/fallback-queue
func (h *SettingHandler) GetFallbackQueueSettings(c *gin.Context) {
	settings, err := h.settingService.GetFallbackQueueSettings(c.Request.Context())
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}

	response.Success(c, dto.FallbackQueueSettings{
		Enabled:            settings.Enabled,
...
// UpdateFallbackQueueSettingsRequest 更新账号兜底排队配置请求
type UpdateFallbackQueueSettingsRequest struct {
	Enabled            bool `json:"enabled"`
	MaxWaiting         int  `json:"max_waiting"`
	WaitTimeoutSeconds int  `json:"wait_timeout_seconds"`
}

// UpdateFallbackQueueSettings 更新账号兜底排队配置
// PUT /api/v1/admin/settings/fallback-queue
func (h *SettingHandler) UpdateFallbackQueueSettings(c *gin.Context) {
	var req UpdateFallbackQueueSettingsRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.BadRequest(c, "Invalid request: "+err.Error())
		return
	}

	settings := &service.FallbackQueueSettings{
		Enabled:            req.Enabled,
		MaxWaiting:         req.MaxWaiting,
		WaitTimeoutSeconds: req.WaitTimeoutSeconds,
	}

	if err := h.settingService.SetFallbackQueueSettings(c.Request.Context(), settings); err != nil {
		response.BadRequest(c, err.Error())
		return
	}

	updatedSettings, err := h.settingService.GetFallbackQueueSettings(c.Request.Context())
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
	...
	response.Success(c, dto.FallbackQueueSettings{
		Enabled:            updatedSettings.Enabled,
		MaxWaiting:         updatedSettings.MaxWaiting,
		WaitTimeoutSeconds: updatedSettings.WaitTimeoutSeconds,
	})
}
