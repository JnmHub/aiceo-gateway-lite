...
func TestSettingHandler_GetPublicSettings_ExposesGatewayLiteUICapabilities(t *testing.T) {
	gin.SetMode(gin.TestMode)
	h := NewSettingHandler(service.NewSettingService(&settingHandlerPublicRepoStub{
		values: map[string]string{},
	}, &config.Config{RunMode: config.RunModeGatewayLite}), "test-version")
	...
	require.NoError(t, json.Unmarshal(recorder.Body.Bytes(), &resp))
	require.Equal(t, 0, resp.Code)
	require.Equal(t, config.RunModeGatewayLite, resp.Data.RunMode)
	require.Equal(t, "gateway-lite-admin", resp.Data.UISurface)
	require.True(t, resp.Data.GatewayLiteAdminEnabled)
	require.False(t, resp.Data.CustomerSelfServiceEnabled)
}
