...
func (h *GatewayHandler) Responses(c *gin.Context) {
	...
	if !h.shouldSkipGatewayLiteBillingEligibility(c) {
		if err := h.billingCacheService.CheckBillingEligibilityForRequest(requestCtx, apiKey.User, apiKey, apiKey.Group, subscription, service.QuotaPlatform(requestCtx, apiKey), service.EstimateRequestTokensForLimit(body)); err != nil {
