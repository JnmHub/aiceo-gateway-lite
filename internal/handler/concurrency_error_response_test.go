...
func TestConcurrencyErrorResponse(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "queue full concurrency error is rate limit",
			err:         &ConcurrencyError{SlotType: "account", IsQueueFull: true},
			slotType:    "user",
			wantStatus:  http.StatusTooManyRequests,
			wantType:    "rate_limit_error",
		...
		{
			name:        "fallback queue full sentinel is rate limit",
			err:         service.ErrFallbackQueueFull,
			slotType:    "account",
			wantStatus:  http.StatusTooManyRequests,
			wantType:    "rate_limit_error",
			wantMessage: "Too many pending requests, please retry later",
