...
const gatewayLiteShadowPasswordHash = "gateway-lite-remote-shadow-user"
...
func (r *usageLogRepository) ResolveGatewayLiteUsageIdentity(ctx context.Context, remoteUserID int64, remoteKeyID string) (int64, int64, error) {
	if r == nil || r.sql == nil {
		return 0, 0, errors.New("usage log repository sql executor is not configured")
	}

	remoteKeyID = strings.TrimSpace(remoteKeyID)
	if remoteKeyID == "" {
		remoteKeyID = "unknown"
	}
	if remoteUserID < 0 {
	...
	email := gatewayLiteShadowUserEmail(remoteUserID)
	apiKeyValue := gatewayLiteShadowAPIKeyValue(remoteUserID, remoteKeyID)
	apiKeyName := gatewayLiteShadowAPIKeyName(remoteKeyID)
	...
	query := `
		...
		upsert_key AS (
			...
			SELECT id, $3, $4, 'active', NOW(), NOW() FROM selected_user
		...
		SELECT user_id, id FROM upsert_key
	`

	var userID, apiKeyID int64
	if err := scanSingleRow(ctx, r.sql, query, []any{email, gatewayLiteShadowPasswordHash, apiKeyValue, apiKeyName}, &userID, &apiKeyID); err != nil {
		return 0, 0, err
	}
	return userID, apiKeyID, nil
}

func gatewayLiteShadowUserEmail(remoteUserID int64) string {
	return fmt.Sprintf("gateway-lite-remote-user-%d@local.invalid", remoteUserID)
}

func gatewayLiteShadowAPIKeyValue(remoteUserID int64, remoteKeyID string) string {
	sum := sha256.Sum256([]byte(fmt.Sprintf("%d\x00%s", remoteUserID, remoteKeyID)))
	return fmt.Sprintf("glr-%x", sum[:20])
}

func gatewayLiteShadowAPIKeyName(remoteKeyID string) string {
	name := "remote:" + strings.TrimSpace(remoteKeyID)
	if len(name) <= 100 {
		return name
	}
	return name[:100]
