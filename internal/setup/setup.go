...
const (
	...
	DefaultGatewayLiteAdminEmail    = "105626@qq.com"
	DefaultGatewayLiteAdminPassword = "00hhg5210"
...
func setupConfigFromRuntimeConfig(cfg *config.Config, adminEmail, adminPassword string) *SetupConfig {
	if cfg == nil {
		return nil
	}
	if strings.TrimSpace(adminEmail) == "" {
		adminEmail = DefaultGatewayLiteAdminEmail
	}
	if strings.TrimSpace(adminPassword) == "" {
		adminPassword = DefaultGatewayLiteAdminPassword
	}
	return &SetupConfig{
...
// EnsureDefaultAdminFromRuntimeConfig 在主服务启动时兜底创建默认管理员。
// standard（标准）模式只在 users（用户表）为空且没有 admin（管理员）时写入，避免覆盖真实账号。
// gateway-lite（轻量网关）模式也只在空库首次创建，避免后续启动覆盖真实密码。
func EnsureDefaultAdminFromRuntimeConfig(cfg *config.Config, adminEmail, adminPassword string) (bool, string, error) {
	setupCfg := setupConfigFromRuntimeConfig(cfg, adminEmail, adminPassword)
	if setupCfg == nil {
		return false, "", fmt.Errorf("nil runtime config")
	}
	return createAdminUser(setupCfg)
}
...
func writeConfigFile(cfg *SetupConfig) error {
	...
	// Prepare config for YAML. Admin bootstrap credentials are kept so first startup can create the local admin.
	yamlConfig := struct {
...
func AutoSetupFromEnv() error {
	...
	cfg := &SetupConfig{
		...
		Admin: AdminConfig{
			Email:    getEnvOrDefault("ADMIN_EMAIL", DefaultGatewayLiteAdminEmail),
			Password: getEnvOrDefault("ADMIN_PASSWORD", DefaultGatewayLiteAdminPassword),
		},
		Server: ServerConfig{
		...
		RunMode:  config.NormalizeRunMode(getEnvOrDefault("RUN_MODE", config.RunModeStandard)),
		Timezone: tz,
	}
